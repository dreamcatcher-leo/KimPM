...
3. **우선순위 3단계 배분 기준 (반드시 3단계 모두 사용)**:

   **P0 — MVP Day1 필수** (전체 기능의 30~40% 목표)
   - 이게 없으면 서비스 자체가 작동하지 않는 기능
   - 첫날 유저가 반드시 사용해야 하는 핵심 플로우
   - 예: 회원가입/로그인, 핵심 데이터 입력, 결제/주문 등

   **P1 — 출시 후 2~4주 이내 필요** (전체 기능의 40~50% 목표)
   - P0가 동작한 후 실제 운영을 지속하기 위해 곧 필요한 기능
   - 없으면 초기 사용자가 이탈하거나 운영이 불편해지는 기능
   - P0보다는 덜 긴급하지만, P2보다는 훨씬 먼저 필요
   - 예: 알림/이메일 발송, 관리자 화면, 검색/필터, 마이페이지 등
   - **대부분의 기능은 P1에 해당합니다. P1을 P0/P2로 극단화하지 마세요.**

   **P2 — 2차 릴리즈 이후** (전체 기능의 10~20% 목표)
   - 서비스 안정화 후 추가해도 충분한 부가 기능
   - 없어도 초기 운영에 지장 없는 편의/고도화 기능
   - 예: 통계 대시보드, 소셜 공유, 고급 커스터마이징, 레거시 마이그레이션 도구 등

   **우선순위 배분 시 주의사항**:
   - P0만 잔뜩, P2만 잔뜩 배분하는 것은 잘못된 분류입니다.
   - 대표가 priority_basis를 입력했으면 해당 기준을 최우선 반영하세요.
   - must_have에 있다고 모두 P0이 아닙니다. P1이 적절한 경우가 많습니다.
   - nice_to_have도 서비스 성격에 따라 P1이 될 수 있습니다.
...
- [ ] P0, P1, P2 세 단계가 모두 존재하는가? (P1이 빠진 경우 반드시 재배분)
- [ ] P0 비율이 30~40%, P1 비율이 40~50%, P2 비율이 10~20% 수준인가?
- [ ] 우선순위 기준(priority_basis)을 최우선 반영했는가?
- [ ] 각 description이 개발팀이 바로 이해할 수 있는 수준으로 구체적인가?`
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: '당신은 외주 개발 PM 전문가입니다. 대표가 입력한 기능 목록을 단 하나도 누락하지 않고, 각각을 개발 가능한 수준으로 구체화합니다. 기능 병합은 절대 하지 않습니다. 우선순위는 P0(30~40%)·P1(40~50%)·P2(10~20%)의 세 단계로 반드시 골고루 배분하며, P1이 빠지는 이분법적 분류(P0/P2만)는 절대 하지 않습니다.',
          },
